...
void echo(int fd)
{
    ...
    if (rio_writen(serverfd, server, strlen(server)) < 0)
        return;
...
int get_index(char *op_uri)
{
    for (int i = 0; i < MAX_CACHE_SIZE / sizeof(Cache_line); i++)
    {
        if (memcmp(cache->blocks[i].uri, op_uri, MAXLINE) == 0)
        {
            return i;
        }
    }
...
int find_LRU()
{
    ...
    for (int i = 0; i < MAX_CACHE_SIZE / sizeof(Cache_line); i++)
    {
        if (cache->blocks[i].valid > 0 && cache->blocks[i].time_stamp > max_stamp)
        {
...
void update(char *op_uri, char *op_object)
{
    ...
    if (i == -1)
        i = find_LRU();
    memcpy(cache->blocks[i].uri, op_uri, MAXLINE);
...
int read_cache(char *op_uri, int fd)
{
    ...
    Rio_writen(fd, cache->blocks[index].object, MAX_OBJECT_SIZE);
    cache->blocks[index].time_stamp = 0;
    update_time_stamp();
