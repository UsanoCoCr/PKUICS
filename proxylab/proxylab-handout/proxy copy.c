...
int get_index(char *op_uri)
{
    for (int i = 0; i < MAX_CACHE_SIZE / sizeof(Cache_line); i++)
    {
        if (cache->blocks[i].valid > 0 && memcmp(cache->blocks[i].uri, op_uri, sizeof(cache->blocks[i].uri)) == 0)
        {
            return i;
        }
    }
...
int read_cache(char *op_uri, int fd)
{
    ...
    Rio_writen(fd, cache->blocks[index].object, sizeof(cache->blocks[index].object));
