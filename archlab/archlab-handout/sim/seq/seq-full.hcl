#/* $begin seq-all-hcl */
# 康子熙 2200017416
# 针对iaddq的修改：iaddq指令和addq指令的区别较小，主要的区别在于iaddq指令使用常数代替了rA，所以在处理iaddq指令时，IIADDQ大部分行为都和IOPQ相同，关于rA的部分删去，同时注意一下iaddq指令会更新条件码即可
# 针对jm的修改： jm指令和jmp的行为也基本相同，只不过jm可以跳到内存，所以我们需要计算valE并赋给valM。最后注意将PC的值设为valM即可
####################################################################
