...
bool get_buildin(struct cmdline_tokens tok);
void do_buildinjob(char **argv);
void kill_wrapper(struct cmdline_tokens tok);
void do_nohup(struct cmdline_tokens tok);
void waitfg(pid_t pid);
bool file_redirect(struct cmdline_tokens *tok);
...
bool get_buildin(struct cmdline_tokens tok)
{
    ...
    if (!strcmp(argv[0], "jobs"))
    {
        ...
        if (tok.outfile != NULL)
        {
            output_fd = open(tok.outfile, O_WRONLY | O_CREAT | O_TRUNC, 0644);
            if (output_fd < 0)
            {
...
void do_buildinjob(char **argv)
{
    ...
    {
        fprintf(stderr, "Error: argument must be a PID or %%jobid\n");
        return;
    }
    else
    {
